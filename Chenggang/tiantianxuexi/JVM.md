...
Full GC 整个堆包括新生代和老年代的垃圾回收

对象优先在Eden区中进行分配，当Eden没有足够空间的时候，虚拟机将会发起一次Minor GC

大对象直接进入老年代，虚拟机中用-XX:PretenureSizeThreshold来定义一个界限，大于这个值的对象都被直接放到老年区。

长期存活的对象将进入老年代，虚拟机给每个对象定义了一个对象年龄计数器（Age），如果对象在Eden出生并经过一次Minor GC后仍然存活，并且能被Survivor容纳，则将对象年龄设为1，每熬过一次Minor GC，年龄就自增一岁，当年龄自增到一定程度（默认是15），会被移动到老年代。-XX:MaxTenuringThreshold可以设置这个年龄界限。

动态年龄的判定，虚拟机并非永远的要求对象年龄到达Threshold之后才进入老年代，如果在Survivor空间中相同年龄所有对象大小的总和大于Survivor的一半，则年龄大于或等于这个年龄的对象就会直接进入老年代。
